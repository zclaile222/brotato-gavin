...
> 生成日期：2026-07-17
> 最近更新：**2026-09-15**（恢复全绿基线、修复敌人池空闲态缺陷，见 §十.1）
> 基于分支：`master`
...
| 主代码量 | 35 个 GDScript 文件，约 **12,707 行** |
| 测试代码量 | 10 个冒烟测试，约 **11,589 行** |
| 场景 | 10 个 `.tscn` 场景 |
| 数据目录 | 8 个 JSON（角色 62 / 武器 78 / 物品 235 / 升级 100） |
| 冒烟测试 | 11 个，**2026-09-15 全量通过**（`bash tests/run_all.sh`） |
| 最大脚本 | `Player.gd` 1910 行、`PlayerUpgrades.gd` 1360 行、`BrotatoData.gd` 1216 行 |
| 分支 | `master` |
...
| 脚本 | 行数 | 说明 |
|------|------|------|
| `scripts/Player.gd` | **1,903** | 严重膨胀，承载大量物品效果逻辑 |
| `scripts/PlayerUpgrades.gd` | **1,359** | 物品效果分发 |
| `scripts/BrotatoData.gd` | **1,110** | 数据加载/验证/转换 |
| `scripts/HUD.gd` | 860 | UI 协调 |
| `scripts/Shop.gd` | 798 | 商店 |
| `scripts/Main.gd` | 712 | 主协调 |
| `scripts/Enemy.gd` | 697 | 敌人 AI |
| `scripts/TurretManager.gd` | 623 | 炮台/结构/召唤物 |
| `scripts/WaveManager.gd` | 528 | 波次状态机 |
| `scripts/PlayerCombat.gd` | **298** | 武器开火，无特殊规则分发 |

### 4.2 架构问题

- **Player.gd 重新成为 God Object**：4 月重构后它一度减到 188 行，但 D4 阶段把每个物品的专用函数直接塞回门面，导致目前比重构前还翻倍。
- **缺少统一 EffectDispatcher**：135 种物品规则靠 `PlayerUpgrades.gd` 字符串分发，65 种武器规则无处可去。
- **PlayerCombat.gd 过薄**：武器个性化行为全部缺失，原版 Brotato 的核心差异主要体现在武器规则上。
...
| **Godot 编辑器手动游玩验证** | **全部未做** | 所有阶段的 Manual verification 复选框均空 |
| Godot headless 沙箱运行 | **正常** | 2026-09-15 实测不再崩溃，**22 个测试可全量 headless 执行**（P4-1 后 18→19，P4-5 后 19→20，单位门后 20→21，战斗目标有效性后 21→22） |
...
| 难度模型 | 简单/普通/困难 + 动态缩放 | Danger 0-5 | 中 |
| Boss 轮换 | **四个 Boss 波各不相同（P4-4 + P4-5 的 P0/P1，2026-09-15）**：第 5 波冰霜 / 第 10 波火焰 / 第 15 波雷电 / 第 20 波均衡型双 Boss。三种变体各有独立数值、弹幕元素与阶段速度倍率 | 每 5 波特定 Boss，20 波双 Boss，各有独特机制 | **小–中**（只差各自的 P2 独特机制） |
...
**尚未实现（设计文档的 P2）**：火焰冲锋 + 燃烧区域（需新建 `BurnZone.tscn`）/
冰霜的冰甲循环 + 冻结脉冲 / 雷电的闪现 + 链式闪电 + 雷暴领域。
目前三个变体之间的差异是 **数值 + 弹幕元素 + 阶段速度倍率**，机制上还没有各自的印记。
...
| P4-2 | 精英词缀系统 | `scripts/Enemy.gd` | 待做 | 现有 `elite_trait` 只有 fast / armored / ghost 三种硬编码特性；需扩为可配置词缀池（再生、反弹、护盾等） |
| P4-3 | Danger 0-5 难度模型 | `scripts/GameState.gd` + `scripts/WaveManager.gd` | 待做 | 替换简单/普通/困难，改为 Danger 等级，影响敌人属性、掉落、解锁 |
...
| P4-5 | 元素 Boss 变体 | `scripts/Enemy.gd` + `scripts/EnemyBullet.gd` + `scripts/PlayerStatus.gd`（新）+ `scenes/BurnZone.tscn` | **P0 + P1 完成，P2 待做（2026-09-15）** | 已完成：三变体的数据定义、弹幕元素、阶段速度倍率差异，以及**玩家侧全新的燃烧/减速/眩晕状态系统**（此前完全不存在）。待做：各自的独特机制（火焰冲锋+燃烧区域 / 冰甲+冻结脉冲 / 闪现+链式闪电+雷暴领域）。见 §7.11 |
...
| 风险 | 影响 | 缓解 |
|------|------|------|
| ~~Godot headless 在沙箱内崩溃~~ | ~~自动化验证受阻~~ | **已消解**：2026-09-15 实测 `--headless` 可正常跑全部 11 个套件，入口为 `bash tests/run_all.sh` |
| `Player.gd` 继续堆积（现 1910 行） | P1/P3 改动冲突 | P1 先只做最小侵入式分发；P3 再重构 |
...
| **1** | **P4-5 剩余：三个 Boss 的独特机制（设计文档的 P2）** | 火焰冲锋+燃烧区域 / 冰甲循环+冻结脉冲 / 闪现+链式闪电+雷暴领域。`docs/boss-variants-design.md` 的 P2 一节已写好每条机制与视觉规格；`scenes/BurnZone.tscn` 待新建。**做完这一步，三个 Boss 才不只是「数值不同的同一个 Boss」** |
| 2 | **P4-2 精英词缀系统** | `Enemy.elite_trait` 目前只有 fast / armored / ghost 三种硬编码特性，需扩为可配置词缀池。注意：波次表已把精英权重从恒定 5.6% 提到第 20 波的 16%，词缀丰富度的收益比改造前更高 |
| 3 | **补齐武器数据的空缺字段**（可选） | 若要让 §7.7 里那 10 条剩余规则中的 9 条也能实现，需要在 `weapons.json` 里补字段（各武器缺什么已在 §7.7 表中列清）。**这是数据设计决策** |
| 4 | **P4-3 Danger 0-5 难度模型** | 替换简单/普通/困难；`GameState.DIFFICULTY_MULTIPLIERS` 与 `WaveManager.get_spawn_interval()` 的缩放口已就位 |
| 5 | **P3 架构还债** | `Player.gd` 仍偏大（实测 1938 行），按计划抽出 `ItemEffects.gd` / `CombatEffects.gd` |
