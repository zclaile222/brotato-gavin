...
| 主代码量 | 41 个 GDScript 文件，**18,397 行** |
| 测试代码量 | 32 个冒烟测试，**20,296 行** |
| 场景 | 12 个 `.tscn` 场景 |
| 数据目录 | 8 个 JSON（角色 62 / 武器 78 / 物品 235 / 升级 100） |
| 冒烟测试 | **32 个**，`tests/run_all.sh` → **PASS=32 FAIL=0**（2026-09-17 实测） |
...
| `scripts/PlayerCombat.gd` | **1,162** | 武器开火 + 规则分发（已从 298 行长回本行） |
...
| **战斗目标失效引用冒烟测试（新增）** | 通过 | `COMBAT_STALE_ENEMY_REFERENCE_SMOKE_PASS`，见 §10.6（Boss 战闪退，玩家实测） |
| ~~`tests/item_runtime_effect_smoke.gd`~~ | **已删除** | 自创建起从未成功运行；调用不存在的静态 API，覆盖范围已由 D4 承担 |
| **Godot 编辑器手动游玩验证** | **仍未做** | 所有阶段的 Manual verification 复选框均空；**商店的可见性缺陷正是靠人眼截图发现的**（见 §7.13），**Boss 战闪退也是玩家实测出来的**（§10.6） |
| Godot headless 沙箱运行 | **正常** | **32 个套件可全量 headless 执行**，PASS=32（2026-09-17 实测） |
...
| 风险 | 影响 | 缓解 |
|------|------|------|
| ~~Godot headless 在沙箱内崩溃~~ | ~~自动化验证受阻~~ | **已消解**：2026-09-17 实测 `--headless` 可正常跑全部 **32** 个套件，入口为 `bash tests/run_all.sh` |
...
| 🔴 **只有实机游玩才会暴露的运行时崩溃** | 打 Boss 战闪退（§10.6），32 个套件在修复前是**全绿**的 | 与上一条同源但机制不同：这次是**类型分叉导致的路径差异** —— 普通敌人走池化、Boss 走 `queue_free()`，而现有测试从没走过「打死 Boss 之后再开火」这条路径。**教训：写测试时要覆盖「对象被释放后仍被引用」这类生命周期边界，而不只是数值正确性**。已补 `combat_stale_enemy_reference_smoke` |
...
| **🔴 打 Boss 战时闪退**（`previously freed`） | 2026-09-17（玩家实测） | `d8f8403` | 详见 §10.6。`fire_weapon` 读到**已释放**的敌人缓存条目。根因是「0.1s 缓存节流」+「Boss 走 `queue_free()` 而普通敌人走 `recycle()`」两条机制叠加 —— **这正是它只在 Boss 战复现的原因** |
...
### 10.6 打 Boss 战时闪退：开火读到已释放的敌人（2026-09-17，玩家实测）

**症状**：玩家在 Boss 战里闪退。`godot.log` 里是**运行时**错误（不是语法错误）：
...
**根因是两条既有机制叠加**（单独看都合理，合起来才致命）：

1. **`_refresh_enemy_cache()` 开头有 0.1s 节流** —— 敌人缓存是**快照**，不是实时视图。
   构建时确实做了 `is_instance_valid` 过滤，但那只能保证「构建那一刻有效」。
2. **`Enemy.die()` 按类型分叉**（`Enemy.gd:776-785`）：

   | 类型 | 回收方式 | 引用是否失效 |
   |---|---|---|
   | Boss / miniboss | `queue_free()` | **帧末真释放 → 失效** |
   | 普通敌人 | `recycle()` | 回池，**永远有效**（只是被移出 `enemies` 组） |

于是：打死 Boss → 缓存里那份引用下一帧变成 `previously freed` → 下次开火读 `.position` 即崩。

> 🔑 **这就是「为什么只有 Boss 战会崩」的答案。** 普通敌人走池化、从不被释放，
> 所以普通战斗怎么打都复现不了。**设计回归测试时必须走 Boss 路径**，
> 如果只用人工 `free()` 造缺陷，就丢掉了「只有 Boss 会释放」这个关键条件。

**修复**（`d8f8403`）：

| 位置 | 改动 |
|---|---|
| `PlayerCombat._live_enemies()`（新） | 开火前就地剪掉失效引用（倒序遍历 + `remove_at`，边删边走不漏项）。同一帧的后续武器不必重复踩同一颗雷 |
| `PlayerCombat.fire_weapon()` | `var nearby = _live_enemies()`（原先直接读 `_cached_enemies`） |
| `PlayerCombat._fire_melee()` | 循环体补 `is_instance_valid` 守卫 |

**为什么近战那条也必须补**：该循环体**自己会打死敌人**（`take_damage` 之后还走命中附带效果，
爆炸 / 连锁可能连带清掉列表里后面的敌人）。若清场走立即 `free()`，**下一次迭代**就在已释放实例上读
`.position` 了。Boss 战爆炸密集，这条路径最先炸 —— 只修 `fire_weapon` 是不够的。

**验证**：`tests/combat_stale_enemy_reference_smoke.gd`（新增，已登记 `run_all.sh`）。
四条判据：① 走**真实死亡路径**（Boss `take_damage` → `die` → `queue_free`）后开火；
② 反向 —— 有效敌人不能被连坐剪掉且仍要能开火（防「过滤过头导致玩家不开火」）；
③ 缓存全失效时提前返回、不开火；④ `_live_enemies()` 只剪失效、不动有效、且幂等。

> 写法上有一处值得记：**前置条件用轮询而不是「固定等 2 帧」。**
> `DetectArea` 的物理重叠要若干帧才稳定，帧数在 headless 下不稳定，
> 固定帧数会让前置条件偶发失败、把测试变成 flaky（本项目在 §10.2 栽过同一类问题）。
> 改为 `_await_in_cache()` 轮询，且失败时打印现场（缓存大小 / 玩家位置 / 战斗阶段 / 探测半径）。

**变异验证（实跑）**：把 `_live_enemies()` 改成不剪枝 → 套件报红 **3 项**，
**并复现出与玩家完全相同的 `SCRIPT ERROR`（`previously freed` at `fire_weapon`）**。
测试非空转，且能忠实复现原始崩溃。

**本次遗留的一个未解项**：玩家同时贴出的那组**语法**错误
（`第 575 行 Expected indented block after "for"`、`第 579 行 Unexpected identifier "nearest"`、
末行 `Expected end of file`）**在磁盘上复现不出来**：

- 逐个扫描 **351 个 `.gd`**（含 4 个孤立 worktree 与归档目录）→ 「for 缺缩进体 = 0、孤立标识符 = 0」
- Godot 自己 `--headless --path . --import` 全量解析 → 无任何 Parse Error

行号（575 / 579）与 `PlayerCombat.gd` **修复前**的编号恰好吻合（旧 575 = `for enemy in nearby:`、
旧 579 = `nearest = enemy`），且末行 `Expected end of file` 是**文件被截断**的典型签名。
**但文件当时与现在都是完整的**，所以无法确认曾发生过截断。
→ 判定为**编辑器侧的陈旧状态**（卡住的脚本缓存 / 未落盘的缓冲），已用 `--import` 重建缓存。
**若用户再次看到该组错误，应先重载工程（Project → Reload Current Project），
仍在则删 `.godot/` 让其重新导入** —— 与 `.git` 那三次异常同属「写入被打断」这一类风险，值得留意。

---

## 九、即时下一步

> 📌 **本节的阅读顺序在 §十 之后**（§十 在第 1133 行、§九 紧跟其后）。
> 这是**既有**的章节错序，不是本次改动引入的。
> 之所以不顺手调换：换行序会让本次同步的 diff 变得几乎无法审阅
> （§十 全文 300+ 行都要显示为改动）。**建议下一次纯文档整理时一并处理。**

基线全绿（**32/32**，2026-09-17 实测，`bash tests/run_all.sh` → `PASS=32 FAIL=0`）。下一步分两条：
...
6. **🔴 打 Boss 战不应再闪退**（§10.6，2026-09-17 修复）：
   重点是在**打死 Boss 之后继续打一会儿**（打下一波、或第 20 波的双 Boss）。
   修复前正是「Boss 被 `queue_free()` → 缓存里那份引用下一帧失效 → 下次开火即崩」。
   **若仍闪退**：把 `%APPDATA%\Godot\app_userdata\Brotato_Gavin\logs\godot.log` 发我 ——
   里面有 `SCRIPT ERROR` 的完整调用栈，能直接定位。
   （顺带留意：玩家同时报了组**语法**错误，但在磁盘上复现不出来，见 §10.6 末段。）
7. 打开工程按 F5 跑一局，**第一波**的旧检查项仍然有效：
   - 屏幕左上角（世界坐标原点 0,0 附近）**不应**有红色方块聚集成群 —— 这是 §10.1 的池缺陷，
     修复前会有 30 个预建敌人在角落里游走向玩家逼近；
   - 玩家不应在视野内没有敌人的情况下持续掉血；
   - 小地图角落不应出现 30 个静止的敌人光点。
8. ~~确认角色选择界面的描述文案没有出现荒谬数值（如「闪避: -3000%」）~~
   **已修（2026-09-15，见 §10.4）**：数据单位已统一为分数，且加了加载期校验门，
   不会再出现这类文本。仍建议顺路扫一眼界面，确认描述整体读起来正常。
9. 波次结束后点击「开始」进入第二波不应报错。
